...
def solution_value(opening_cost,demand,capacity,travel_cost,centers,assignments):
    flag = True
...
def check_functions():
    opening_cost,demand,capacity,travel_cost = read_instance(INSTANCE)
    sol1 = minimum_travel_cost(demand,adapt_capacity(CENTERS,capacity),travel_cost)
    print("Solution minimum_travel_cost {}".format(sol1))
    if len(sol1[1]) == 0:
        print("Erreur, pas de solution retournée")
    else:
        sol1_check = solution_value(opening_cost,demand,capacity,travel_cost,CENTERS,sol1[1])
        ...
        if sol1_check == -1:
            ...
            if sol1[0] + centers_cost(opening_cost) != sol1_check:
                ...
                print("Solution trouvée meilleure que la solution optimale prévue... contactez l'assistant")
            else:
                print("Solution optimale trouvé")

    sol2 = facility_location(opening_cost,demand,capacity,travel_cost)
    print("Solution facility_location {}".format(sol2))
    if len(sol2[2]) == 0:
        print("Erreur, pas de solution retournée")
    else:
        sol2_check = solution_value(opening_cost,demand,capacity,travel_cost,sol2[1],sol2[2])
